<script lang="ts">
    ...
    // Check if ANY wallet is working
    $: isAnyWorking = ($stats.wallets || []).some(w => w.working);
    // Check if network connection is okay (independent of mining status)
    $: isOnline = $connected; 

    // Combined status for Badge
    ...
    async function toggleGlobal() {
        ...
        if (newState) {
            ...
            notifications.info("Всі воркери зупинено. Оновлення балансів активне.");
        }
    }
...
<div class="content-wrapper">
    ...
    <div class="dash-header" style="flex-shrink: 0;">
        ...
        <div style="display: flex; gap: 15px; align-items: center;">
            <div class="dash-controls">
                <button class="btn-icon" on:click={toggleGlobal} title={isAnyWorking ? "Зупинити всі" : "Запустити всі"} style="width: auto; padding: 0 20px; font-weight: 600; gap: 8px;">
                    <i class="fas {isAnyWorking ? 'fa-stop' : 'fa-play'}" style="color: {isAnyWorking ? 'var(--danger)' : 'var(--success)'}"></i>
    ...
    <div class="grid-3" style="flex-shrink: 0;">
        ...
        <div class="glass-card stat-card" style="border-top-color: var(--success);">
            <i class="fas fa-wallet stat-icon-bg"></i>
            <div class="stat-label">Загальний баланс</div>
            <div class="stat-value" style="color: var(--success);">{$stats.total_balance.toFixed(2)}</div>
    ...
    <div class="chart-grid">
        <div class="glass-card chart-section no-hover" style="grid-column: 1; display: flex; flex-direction: column; height: 100%;">
            <div style="display:flex; justify-content:space-between; margin-bottom:10px; flex-shrink: 0;">
                ...
                <button class="btn-icon" on:click={openFocus} title="Відкрити режим фокусу">
                    <i class="fas fa-expand"></i>
                </button>
            </div>
            <div style="flex:1; position:relative; width: 100%; min-height: 0; overflow: hidden;">
                <canvas bind:this={chartCanvas} style="width: 100%; height: 100%; display: block;"></canvas>
            </div>
        </div>
        
        <div class="glass-card recent-logs no-hover" style="grid-column: 2; display: flex; flex-direction: column; height: 100%; max-height: none;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-shrink: 0;">
                <div class="stat-label">Остання активність</div>
            </div>
            <div id="quick-stats-container" style="flex: 1; overflow-y: auto; min-height: 0;">
                 {#each $logs.slice().reverse().slice(0, 50) as log}
                    <div class="log-row-mini {log.type}">
                        <span style="opacity:0.5; margin-right: 8px; font-size: 0.8em;">{log.time}</span>
                        <span>{log.message}</span>
                    </div>
                 {/each}
                 {#if $logs.length === 0}
                    <div class="wallet-select-placeholder" style="padding: 20px; text-align: center; color: #64748b; border: none; background: transparent;">
    ...
    <!-- Fun Stats - Fixed at bottom -->
    <div class="glass-card no-hover" style="flex-shrink: 0; padding: 20px; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; text-align: center;" id="fun-stats-section">
        <div style="padding: 10px; background: rgba(129, 140, 248, 0.05); border-radius: 12px; border: 1px solid rgba(129, 140, 248, 0.1);">
            <div style="font-size: 1.5rem; font-weight: 800; font-family: var(--font-mono); color: var(--primary); margin-bottom: 4px;">{$stats.hashrate.toFixed(2)}</div>
            <div style="font-size: 0.75rem; color: #94a3b8; text-transform: uppercase;">Швидкість</div>
        </div>
        <div style="padding: 10px; background: rgba(16, 185, 129, 0.05); border-radius: 12px; border: 1px solid rgba(16, 185, 129, 0.1);">
        ...
        <div style="padding: 10px; background: rgba(6, 182, 212, 0.05); border-radius: 12px; border: 1px solid rgba(6, 182, 212, 0.1);">
            ...
            <div style="font-size: 0.75rem; color: #94a3b8; text-transform: uppercase;">Активні</div>
        </div>
    </div>
</div>
