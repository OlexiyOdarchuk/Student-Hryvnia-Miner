...
* 🪟 **Кросплатформність** — однаково добре працює на Windows, Linux. Має повноцінний NSIS інсталятор для Windows. (версія не MacOS не тестована і ви можете зібрати її тільки власноруч)
...
Для більшості користувачів найпростіший спосіб почати — завантажити вже зібраний файл для вашої операційної системи з розділу **[Releases](https://github.com/OlexiyOdarchuk/Student-Hryvnia-Miner/releases/)**.

**Для Windows доступні дві версії:**
* `SHMiner-windows-amd64-setup.exe` — **Рекомендовано**. Повноцінний інсталятор, який створює ярлики, безпечно оновлює програму та має вбудований деінсталятор.
* `SHMiner-windows-amd64-portable.exe` — Портативна версія, що не потребує встановлення.

**Для Linux:**
* `SHMiner-linux-amd64` — Готовий бінарний файл.
...
* [nsis](https://nsis.sourceforge.io/Download) (тільки якщо ви хочете зібрати інсталятор для Windows)
...
3. Збірка для продакшну: `wails build -clean -trimpath -ldflags "-s -w"` (додайте прапорець `-nsis` для створення інсталятора Windows). Готовий файл буде знаходитися в директорії `build/bin`.

## 🎮 Використання

1. **Запустіть додаток**: Відкрийте встановлену програму, портативний файл або виконайте `wails dev`.
2. **Авторизація**: Задайте пароль для шифрування ваших даних. *Якщо ви не хочете вводити пароль щоразу, просто залиште поле порожнім під час реєстрації — додаток входитиме автоматично, а пароль можна додати потім, при необхідності.*
...
* [ ] Додати пакет в `AUR` і зробити `.deb` та `AppImage` пакети для Linux-дистрибутивів.
